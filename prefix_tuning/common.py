...
def build_prompt(meta, repo_context, user_query):
    return (
        "### METADATA\n"
...
def contains_any_substring(text, patterns):
    if not patterns:
        return True
    text = text.lower()
...
def matches_condition(meta, cond):
...
class PrefixDataset(Dataset):
    def __init__(self, examples, tokenizer, max_length = 2048) -> None:
...
class CausalCollator:
    ...
    def __call__(self, features):
        ...
        for x in features:
            ...
            batch["input_ids"].append(torch.cat([x["input_ids"], torch.full((pad_len,), self.pad_token_id, dtype=torch.long)]))
            batch["attention_mask"].append(torch.cat([x["attention_mask"], torch.zeros(pad_len, dtype=torch.long)]))
            batch["labels"].append(torch.cat([x["labels"], torch.full((pad_len,), -100, dtype=torch.long)]))
        return {k: torch.stack(v) for k, v in batch.items()}

def filter_examples_for_adapter(examples, spec):
    return [ex for ex in examples if matches_condition(ex["meta"], spec.route_condition)]

def synthesize_routing_rules(adapter_specs):
    rules = []
...
def default_adapter_specs(root = "./adapters"):
    return [
        AdapterSpec(
            name="graph_extractor",
            output_dir=f"{root}/graph_extractor",
            route_condition=RouteCondition(
