...
from prefix_tuning.common import PrefixDataset, CausalCollator, filter_examples_for_adapter, synthesize_routing_rules, default_adapter_specs, AdapterSpec
...
def compute_node_recall(predicted, ground_truth):
...
def compute_edge_precision(predicted, ground_truth):
...
def evaluate_graph_quality(predicted_json, ground_truth_json):
    try:
...
def train_one_adapter_with_metrics(base_model_name, tokenizer, examples, spec, val_examples = None):
...
def main():
    parser = argparse.ArgumentParser(description="Train graph-specialized prefix tuning adapters with quality metrics")
