...
For each experiment with a condition_b/graph_final.json, counts node types
...
def count_nodes(graph_path: Path) -> dict[str, int]:
...
def main() -> None:
    ...
    for row in rows:
        ...
        print(f"| {row['project']} | " + " | ".join(counts) +
              f" | {row['total_nodes']} | {row['total_edges']} |")
