...
def validate_training_examples(input_file = "data/training_examples.json"):
...
def print_validation_report(stats):
    ...
    fw_sorted = sorted(stats.get("framework_distribution", {}).items(), key=lambda x: x[1], reverse=True)[:10]
...
def save_validation_report(stats, output_file = "data/training_data_stats.json"):
